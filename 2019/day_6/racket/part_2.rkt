...
(define (dfs g looking-for current path)
  (if (equal? looking-for current)
      path
      (if (not (hash-has-key? (graph-nodes g) current))
          (list)
          (let* ([n         (hash-ref (graph-nodes g) current)]
                 [children  (node-children n)]
                 [new-path  (append path (list current))])
            (flatten (map (lambda (c) (dfs g looking-for c new-path)) children))))))
 
(define (path-to g dst)
  (dfs g dst "COM" (list)))

(define (-last-common-parent path-one path-two last)
  (let ([n1    (car path-one)]
        [n2    (car path-two)]
        [rest1 (cdr path-one)]
        [rest2 (cdr path-two)])
    (if (equal? n1 n2)
        (-last-common-parent rest1 rest2 n1)
        last)))

(define (last-common-parent path-one path-two)
  (-last-common-parent path-one path-two "###ROOT###"))

(define (dist-from parent path)
  (let ([parent-index (index-of path parent)])
    (- (length path) (+ 1 parent-index))))
...
(define (solution)
  (let* ([g (graph-from-input)]
         [path-to-you   (path-to g "YOU")]
         [path-to-san   (path-to g "SAN")]
         [common-parent (last-common-parent path-to-you path-to-san)]
         [you-dist      (dist-from common-parent path-to-you)]
         [san-dist      (dist-from common-parent path-to-san)])
    (println (number->string (+ you-dist san-dist)))))
