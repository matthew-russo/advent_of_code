...
(define (run-simulation src a-setting b-setting c-setting d-setting e-setting)
  (let*-values ([(a-in e-out)  (make-pipe)]
    ...
    (println a-setting e-out)
    (println b-setting a-out)
    (println c-setting b-out)
    (println d-setting c-out)
    (println e-setting d-out)
    (println 0 e-out)
    (thread (lambda () (run-interpreter a-interp)))
    (thread (lambda () (run-interpreter b-interp)))
    (thread (lambda () (run-interpreter c-interp)))
    (thread (lambda () (run-interpreter d-interp)))
    (thread-wait (thread (lambda () (run-interpreter e-interp))))
    (read a-in)))
